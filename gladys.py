...
def main():
    ...
    input(f"{characters["-player"]}When that thing first came to this kingdom, life over here changed ")
    input(f"{characters['-player']}No one comes out anymore, the air is ashy, the sky is dark, the mood is gloomy ")
    print(" ")
    input(f"{characters['-player']}* Sigh *")
    print(" ")
    input(f"{characters['-player']}We as knights have to finish this thing off ")
    input(f"{characters['-player']}We are the last line of hope for this kingdom ")
    input(f"{characters['person01']}It's scary to think about it ")
    input(f"{characters['person01']}We were inducted as knights only 7 months ago ")
    input(f"{characters['person01']}We were barely on our first excursion when this beast came and terrorized us ")
